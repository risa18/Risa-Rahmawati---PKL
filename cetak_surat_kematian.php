...
<head>
    <meta charset="utf-8" />
    <title>Web Apps | Surat Kematian</title>
...
<body>
    ...
    $no = 1;
    $id = $_GET['id'];
    $result = $koneksi->query("SELECT * FROM data_kematian JOIN data_penduduk ON data_kematian.id_penduduk = data_penduduk.id WHERE data_kematian.id = $id");
    $no = 1;
    if ($result->num_rows > 0) :
        while ($row = $result->fetch_assoc()) {
    ?>

            <h4 style="text-align: center; margin-bottom: 10px;"><u>Surat Kematian</u></h4>
            ...
            <justify>
                <p> &emsp;
                    <b><i>Telah Meninggal Dunia</i></b> dengan keterangan yang diketahui sebagai berikut :
                </p>
                <p>&emsp; &emsp; &emsp; Meninggal pada tanggal &emsp; &emsp; : <?= date('d M Y', strtotime($row['tanggal_kematian'])) ?></p>
                <p>&emsp; &emsp; &emsp; Dengan sebab &ensp; &emsp; &emsp; &emsp; &emsp; &emsp;: <?= $row['sebab_kematian'] ?></p>
                <p> &emsp; Demikian <b>SURAT KEMATIAN</b> ini kami buat atas dasar yang sebenarnya kepada pihak terkait mohon maklum adanya.</p>
                <br />
            </justify>
